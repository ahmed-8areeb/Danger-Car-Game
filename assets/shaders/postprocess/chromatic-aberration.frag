...
void main(){
    ...
    vec4 rightTex=texture(tex,vec2(tex_coord.x+STRENGTH,tex_coord.y));
    vec4 leftTex=texture(tex,vec2(tex_coord.x-STRENGTH,tex_coord.y));
    frag_color = vec4( leftTex.x, originalTex.y,rightTex.z, originalTex.w);
}